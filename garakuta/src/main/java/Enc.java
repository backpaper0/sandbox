...
public class Enc {
	...
	private static void enc(final String key, final InputStream in, final OutputStream out) throws Exception {
	...
	private static void dec(final String key, final InputStream in, final OutputStream out) throws Exception {
		final ByteArrayInputStream bais = new ByteArrayInputStream(Base64.getUrlDecoder().decode(in.readAllBytes()));
		final Cipher c = Cipher.getInstance("AES/CBC/PKCS5Padding");
		final byte[] iv = new byte[c.getBlockSize()];
		bais.read(iv);
