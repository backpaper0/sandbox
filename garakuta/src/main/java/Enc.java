...
public class Enc {
	...
	private static void enc(String key, InputStream in, OutputStream out) throws Exception {
	...
	private static void dec(String key, InputStream in, OutputStream out) throws Exception {
		ByteArrayInputStream bais = new ByteArrayInputStream(Base64.getUrlDecoder().decode(in.readAllBytes()));
		Cipher c = Cipher.getInstance("AES/CBC/PKCS5Padding");
		byte[] iv = new byte[c.getBlockSize()];
		bais.read(iv);
