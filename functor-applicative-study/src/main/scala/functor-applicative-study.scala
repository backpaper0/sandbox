...
trait Applicative[A, F[_]] { self: F[A] =>
  def <*>[B](f: F[A => B]): F[B]
}

sealed trait Maybe[A] extends Functor[A, Maybe] with Applicative[A, Maybe]
...
case class Nothing[A]() extends Maybe[A] {
  ...
  def <*>[B](f: Maybe[A => B]): Maybe[B] = Nothing()
}

sealed trait List[A] extends Functor[A, List] with Applicative[A, List]

case class Cons[A](head: A, tail: List[A]) extends List[A] {
  def fmap[B](f: A => B): List[B] = Cons(f(head), tail.fmap(f))
  def <*>[B](f: List[A => B]): List[B] = {
